      ...
      try {
        ...
        if (res.status === '200') {
          if (deleteButton) {
            ...
            await this.card.querySelector(cardButton).classList.add(buttonTypeBookmark);
            await this.card.querySelector(cardButton).classList.remove(buttonTypeBookmarkMarked);
          }
        } else {
    ...
    if (this.card.querySelector(`.${buttonTypeBookmark}`) && storage.getStorageItem('username')) {
      try {
        ...
        if (res.status === '201') {
          ...
          await this.card.querySelector(cardButton).classList.remove(buttonTypeBookmark);
          await this.card.querySelector(cardButton).classList.add(buttonTypeBookmarkMarked);
          return;
        }
